/**
 * ManageDisciplinesModal — admin CRUD for the disciplines collection.
 * Modeled exactly on ManageCategoriesModal.
 */
...
export default function ManageDisciplinesModal({ disciplines: initial, onClose, onChange }: Props) {
  const [items, setItems]           = useState<Discipline[]>(initial)
  ...
  function startEdit(d: Discipline) {
    setEditingId(d.id)
    setEditValues({ name: d.name, emoji: d.emoji })
    setError('')
  }
  ...
  async function saveEdit(id: string) {
    ...
    try {
      const res = await api.patch<{ status: string; data: Discipline }>(
        `/api/disciplines/${id}`,
        { name: editValues.name.trim(), emoji: editValues.emoji.trim() },
      )
      const updated = items.map((d) => (d.id === id ? res.data : d))
      setItems(updated)
      onChange(updated)
      setEditingId(null)
    } catch (err) {
  ...
  async function handleAdd() {
    ...
    setSaving(true); setError('')
    try {
      const res = await api.post<{ status: string; data: Discipline }>(
        '/api/disciplines',
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-4 space-y-2 flex-1">
          {items.length === 0 && (
            <p className="text-sm text-muted text-center py-6">No disciplines yet. Add one below.</p>
          )}

          {items.map((disc) => (
            <div key={disc.id} className="rounded-xl border-2 border-surface-2 overflow-hidden">
              {editingId === disc.id ? (
                <div className="p-3 space-y-2">
                  ...
                  <p className="text-xs text-muted">
                    Slug (cannot change):{' '}
                    <code className="bg-gray-100 px-1 rounded font-mono">{disc.id}</code>
                  </p>
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => saveEdit(disc.id)}
                      disabled={saving}
                ...
                <div className="flex items-center gap-3 px-4 py-3">
                  <span className="text-xl w-7 text-center shrink-0">
                    {disc.emoji || <span className="text-gray-300 text-sm">—</span>}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-ink truncate">{disc.name}</p>
                    <p className="text-xs text-muted font-mono">{disc.id}</p>
                  </div>
